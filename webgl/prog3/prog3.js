...
function init()
{
    ...
    normals = []; // set normal array to an empty set

    for (var i = 0, len = vertices.length; i < len; i++) // set earch normal vector to [0, 0, 0]
    {
        normals[i] = [0, 0, 0];
    }

    for (var i = 0, len = triangles.length; i< len; i++) // Find the normal for each triangle and add it to each adjacent vertex.
    {
        var a = normalize(sub(vertices[triangles[i][1]], vertices[triangles[i][0]]));
        var b = normalize(sub(vertices[triangles[i][2]], vertices[triangles[i][0]]));
    ...
    for (var i = 0, len = normals.length; i < len; i++) // Renormalize to ensure that all new normal vectors have unit length
    {
